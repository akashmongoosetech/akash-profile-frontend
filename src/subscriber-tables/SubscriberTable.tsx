...
const SubscriberTable: React.FC<SubscriberTableProps> = ({ className = '', onDataChange }) => {
  ...
  const getStatusInfo = (status: string) => {
    const statusConfig = {
      active: { color: 'text-green-400', bg: 'bg-green-400/10 border border-green-400/20 shadow-[0_0_10px_rgba(74,222,128,0.2)]', icon: CheckCircle },
      unsubscribed: { color: 'text-red-400', bg: 'bg-red-400/10 border border-red-400/20 shadow-[0_0_10px_rgba(248,113,113,0.2)]', icon: XCircle },
      pending: { color: 'text-yellow-400', bg: 'bg-yellow-400/10 border border-yellow-400/20 shadow-[0_0_10px_rgba(250,204,21,0.2)]', icon: Mail },
    };
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-700/50">
              {currentSubscribers.map((subscriber) => {
                ...
                return (
                  <tr key={subscriber._id} className="hover:bg-slate-700/30 transition-colors group">
