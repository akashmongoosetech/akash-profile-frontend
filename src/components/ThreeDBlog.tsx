...
function AnimatedCylinder() {
  ...
  return (
    <Float floatIntensity={1.5} speed={1.3}>
      <mesh ref={ref} castShadow receiveShadow>
        <cylinderGeometry args={[1.2, 1.2, 2, 32]} />
        <meshStandardMaterial color="rebeccapurple" metalness={0.6} roughness={0.2} />
      </mesh>
    </Float>
  );
}

const ThreeDBlog: React.FC = () => (
  <Canvas camera={{ position: [0, 0, 8], fov: 60 }} style={{ height: '100vh', width: '100vw' }} shadows>
    <ambientLight intensity={0.6} />
    <directionalLight position={[5, 10, 5]} intensity={1.2} castShadow />
    <pointLight position={[-10, -10, -10]} color="rebeccapurple" intensity={0.5} />
    ...
    <Html position={[0, -2.5, 0]} center>
      <h2 style={{ color: 'white', fontSize: '1.5rem' }}>Blog</h2>
      <p style={{ color: 'white' }}>This is my 3D Blog page!</p>
    </Html>
    <OrbitControls />
  </Canvas>
);

export default ThreeDBlog; 