...
function AnimatedTorus() {
  ...
  return (
    <Float floatIntensity={1.7} speed={1.5}>
      <mesh ref={ref} castShadow receiveShadow>
        <torusGeometry args={[1.2, 0.4, 32, 100]} />
        <meshStandardMaterial color="crimson" metalness={0.7} roughness={0.25} />
      </mesh>
    </Float>
  );
}

const ThreeDContact: React.FC = () => (
  <Canvas camera={{ position: [0, 0, 8], fov: 60 }} style={{ height: '100vh', width: '100vw' }} shadows>
    <ambientLight intensity={0.6} />
    <directionalLight position={[5, 10, 5]} intensity={1.2} castShadow />
    <pointLight position={[-10, -10, -10]} color="crimson" intensity={0.5} />
    ...
    <Html position={[0, -2.5, 0]} center>
      <h2 style={{ color: 'white', fontSize: '1.5rem' }}>Contact</h2>
      <p style={{ color: 'white' }}>This is my 3D Contact page!</p>
    </Html>
    <OrbitControls />
  </Canvas>
);
